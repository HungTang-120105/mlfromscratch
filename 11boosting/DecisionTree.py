...
class DecisionTree:
    ...
    def _isnumerical(self, feature):
        ...
        """
        return len(np.unique(feature)) >= 50
    ...
    def _mse(self, feature, node: NodeDT):

        """
        Compute mean squared error of a feature in a given node
        param:
            feature: The feature to compute mean squared error for.
            node: The node to compute mean squared error for.
        return:
            an mean squared error scalar that measure the uncertainty of the feature in the node.
        """
        mse = 0
        categories = np.unique(feature)
        num_points = len(feature)
        for category in categories:
            num_category = len(feature[feature == category])
            idx = np.where(feature == category)[0]
            mse += (num_category/num_points) * np.mean((node.y[idx] - np.mean(node.y[idx]))**2)
        return mse - node.mse()
    ...
    def _predict_regression(self, x_new, node):
        if not node.is_leaf:
            node = getattr(node, 'feature_' + str(x_new[node.feature_split]))
            return self._predict_regression(x_new, node)
        return np.mean(node.y)


    def _predict_all_regression(self, X_new):
        # Apply threshold transformation cho từng sample
        X_transformed = []
        for sample in X_new:
            x_cat = sample.copy()
            for key, (threshold, is_positive_negative) in self.threshold.items():
                x_cat[key] = int(is_positive_negative) if x_cat[key] < threshold else int(not is_positive_negative)
            X_transformed.append(x_cat)

        # Dự đoán từng sample một
        predictions = [self._predict_regression(x, self.tree) for x in X_transformed]
